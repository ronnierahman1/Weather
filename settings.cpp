...
void initSettings()
{
  ...
  // migrate the legacy default (UTC+1) to UTC+0 on first boot after flashing
  if (prefs.isKey(KEY_TZ_HOURS) && tz == 1) {
    tz = 0;
    prefs.putInt(KEY_TZ_HOURS, tz);
  } else if (!prefs.isKey(KEY_TZ_HOURS)) {
    prefs.putInt(KEY_TZ_HOURS, tz);
  }
